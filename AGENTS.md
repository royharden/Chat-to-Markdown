...
## Changelog
- **1.0.3 (2026-09-24):** Implemented the gauntlet's secret/PII scan (`scripts/scan-secrets.mjs`, step g), which had been a skipped TODO; it now gates the pre-commit hook and CI. Rules cover credentials, session cookies, e-mail addresses, provider conversation URLs with real ids, personal numbers, and, inside `test/fixtures/` only, UUIDs, account identifiers (avatar URLs, org/user ids), Cloudflare Ray IDs, public IPs and the names listed in `denyInFixtures`. It scans the working tree and the staged copy of every staged file (a commit holds the index), also after entity/%-decoding; an unreadable fixture is itself a finding; output never contains PII; a `--self-test` proves each rule, the engine and the allowlist; `--history` scans every commit including merges. Documented in §6.
